...
function startFn() {
  ...
  function createPlayers(name, mark) {
    return players(name, mark);
  }

  const player1 = createPlayers("player X", "X");
  const player2 = createPlayers("player O", "O");
  ...
  function gameBoard() {
    ...
    function printWinner() {
      if (gameFlag) {
        ...
        !gameFlag &&
        activePlayer.getMoves().length +
          switchActivePlayer().getMoves().length ==
          9
      ) {
        return "It's a draw";
      } else {
